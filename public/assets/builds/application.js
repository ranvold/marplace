    ...
    $2.extend(Datepicker.prototype, {
      ...
      _determineDate: function(inst, date, defaultDate) {
        var offsetNumeric = function(offset2) {
          ...
          var date2 = (offset2.toLowerCase().match(/^c/) ? $2.datepicker._getDate(inst) : null) || new Date(), year = date2.getFullYear(), month = date2.getMonth(), day = date2.getDate(), pattern = /([+\-]?[0-9]+)\s*(d|D|w|W|m|M|y|Y)?/g, matches = pattern.exec(offset2);
          while (matches) {
            switch (matches[2] || "d") {
              case "d":
              case "D":
